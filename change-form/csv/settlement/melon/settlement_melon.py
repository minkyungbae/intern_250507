...
# ownershipUser 문자열에서 각 unique id에 대해 UUID 생성
def split_and_generate_uuids(text):
    if pd.isna(text):
        return None
    text = str(text).strip()
    names = text.split(' , ')
    uuid_list = [generate_uuid_from_text(name) for name in names]
...
def process_sheet(file_path, output_path):
    ...
    # 특정 칼럼의 null 값을 '특정 문자'로 채우기
    df['ownershipUser'] = df['ownershipUser'].fillna('0000000000000x000000000000000004')
    
    """
    1. Creation Date -> created_date
    2. Modified Date -> modified_date
    3. id 컬럼은 맨 뒤에 추가
    4. ownershipUser -> ownershipUser_formatted
    """

    # 이전 칼럼 오른쪽에 새로운 컬럼 추가
    ...
    # 'id' 컬럼은 마지막 열에 추가
    df['id'] = None
    df.insert(df.columns.get_loc('ownershipUser') + 1, 'ownershipUser_formatted', pd.NA)
    ...
    for idx, row in df.iterrows():
        ...
        # ownershipUser 컬럼을 토대로 uuid를 ownershipUser_formatted에 추가
        row_label = row.get('ownershipUser')
        if pd.notna(row_label):
            label_str = str(row_label).strip()
            if label_str:
                df.at[idx, 'ownershipUser_formatted'] = split_and_generate_uuids(label_str)
