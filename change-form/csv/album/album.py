...
def process_sheet(file_path, output_path):
    ...
    df['label'] = df['label'].fillna('0000000000000x000000000000000001')
    
    """
    1. Creation Date -> created_date
    2. Modified Date -> modified_date
    3. id 컬럼은 맨 뒤에 추가
    4. label -> label_formatted
    """
    ...
    # 'id' 컬럼은 마지막 열에 추가
    df['id'] = None
    df.insert(df.columns.get_loc('label') + 1, 'label_formatted', pd.NA)
    ...
    for idx, row in df.iterrows():
        ...
        # label 컬럼을 토대로 uuid를 label_formatted에 추가
        row_label = row.get('label')
        if pd.notna(row_label):
            label_str = str(row_label).strip()
            if label_str:
                df.at[idx, 'label_formatted'] = generate_uuid_from_text(label_str)
