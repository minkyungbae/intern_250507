...
# channels 문자열에서 각 unique id에 대해 UUID 생성
...
def process_sheet(file_path, output_path):
    ...
    df['channels'] = df['channels'].fillna('0000000000000x000000000000000006')
    
    """
    1. Creation Date -> created_date
    2. Modified Date -> modified_date
    3. id 컬럼은 맨 뒤에 추가
    4. channels -> channels_formatted
    """
    ...
    # 'id' 컬럼은 마지막 열에 추가
    df['id'] = None
    df.insert(df.columns.get_loc('channels') + 1, 'channels_formatted', pd.NA)
    ...
    for idx, row in df.iterrows():
        ...
        # channels 컬럼을 토대로 uuid를 channels_formatted에 추가
        row_label = row.get('channels')
        if pd.notna(row_label):
            label_str = str(row_label).strip()
            if label_str:
                df.at[idx, 'channels_formatted'] = split_and_generate_uuids(label_str)
