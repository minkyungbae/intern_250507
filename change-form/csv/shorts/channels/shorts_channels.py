...
def process_sheet(file_path, output_path):
    ...
    df['contract'] = df['contract'].fillna('0000000000000x000000000000000004')
    
    """
    1. Creation Date -> created_date
    2. Modified Date -> modified_date
    3. id 컬럼은 맨 뒤에 추가
    4. contract -> contract_formatted
    """
    ...
    # 'id' 컬럼은 마지막 열에 추가
    df['id'] = None
    df.insert(df.columns.get_loc('contract') + 1, 'contract_formatted', pd.NA)
    ...
    for idx, row in df.iterrows():
        ...
        # contract 컬럼을 토대로 uuid를 contract_formatted에 추가
        row_label = row.get('contract')
        if pd.notna(row_label):
            label_str = str(row_label).strip()
            if label_str:
                df.at[idx, 'contract_formatted'] = generate_uuid_from_text(label_str)
